...
run ::
    (MonadLogger m, MonadBaseControl IO m, MonadIO m, Backend backend) =>
    backend ->
    Client api ->
    m ()
run b c = do
...
getServerResponse b req sendResponse = void $
    requestResource b req $
        \httpResponse -> do
            ...
            sendResponse $
                if isGoodStatus
                    then Resp.Modified req response
                    else Resp.HttpRequestFailed req httpResponse
